...
impl App {
    ...
    pub fn ui(&mut self, ui: &mut render::Canvas) {
        ...
        let widgets = [
            // menubar
            Flow::with_height(20.0).expand_across(),
            // contextbar
            Flow::with_height(20.0).expand_across(),
        ...
        let mut iter = ui.horizontal_flow(0.0, 0.0, &widgets);

        let menubar = iter.next().unwrap();
        let contextbar = iter.next().unwrap();
        let timeline = iter.next().unwrap();
        let statusbar = iter.next().unwrap();
