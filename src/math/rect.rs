...
impl<S> Rect<S> where S: BaseNum {
    ...
    pub fn from_min_dim(min: Point2<S>, dim: Vector2<S>) -> Self {
        Self::from_min_max(min, min + dim)
    }
    pub fn from_coords(x1: S, y1: S, x2: S, y2: S) -> Self {
        Self::from_min_max(Point2::new(x1, y1), Point2::new(x2, y2))
    }
    pub fn from_coords_and_size(x: S, y: S, w: S, h: S) -> Self {
        Self::from_min_dim(Point2::new(x, y), Vector2::new(w, h))
    }
}
...
impl<S> Rect<S> where S: BaseNum {
    pub fn shift_x(self, x: S) -> Self {
        self.shift_xy(Vector2::new(x, S::zero()))
    }
    pub fn shift_y(self, y: S) -> Self {
        self.shift_xy(Vector2::new(S::zero(), y))
    }
    pub fn shift_x_y(self, x: S, y: S) -> Self {
        self.shift_xy(Vector2::new(x, y))
    }
